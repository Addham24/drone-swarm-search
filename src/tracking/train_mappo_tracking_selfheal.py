...
def env_creator(args):
    print("-------------------------- MAPPO TRACKING SELFHEAL ENV CREATOR --------------------------")
...
if __name__ == "__main__":
    ...
    env_name = "DSSE_Tracking_MAPPO_SelfHeal"

    register_env(env_name, lambda config: ParallelPettingZooEnv(env_creator(config)))
    ModelCatalog.register_custom_model("MAPPOCNNModel_Tracking_SelfHeal", MAPPOCNNModel)
    ...
    config = (
        ...
        .training(
            ...
            num_sgd_iter=10,
            model={
                "custom_model": "MAPPOCNNModel_Tracking_SelfHeal",
    ...
    exp_name = os.environ.get("EXP_NAME", "mappo_selfheal")
    curr_path = pathlib.Path(__file__).resolve().parent.parent

    tune.run(
        "PPO",
        name="MAPPO_Tracking_SelfHeal_" + exp_name,
