"""
RSPO V2 SelfHeal Training Script for Dynamic Target Tracking
=============================================================
RSPO V2 training on DSSE.DroneSwarmSearch (Dynamic Tracking)
WITH teammate compensation rewards (Self-Healing Environment).

Usage:
    python src/tracking/train_rspo_v2_tracking_selfheal.py
"""
...
def env_creator(args):
    print("-------------------------- RSPO V2 TRACKING SELFHEAL ENV CREATOR --------------------------")
...
if __name__ == "__main__":
    ...
    env_name = "DSSE_Tracking_RSPO_V2_SelfHeal"

    register_env(env_name, lambda config: ParallelPettingZooEnv(env_creator(config)))
    ModelCatalog.register_custom_model("RSPOModelV2_Tracking_SelfHeal", RSPOModelV2)
    ...
    print(f"[RSPO V2 Tracking SelfHeal] Detected {total_cpus} CPUs. Configured with {num_gpus} GPUs and {num_runners} environment runners.")
    ...
    config = (
        ...
        .training(
            ...
            num_sgd_iter=10,
            model={
                "custom_model": "RSPOModelV2_Tracking_SelfHeal",
    ...
    exp_name = os.environ.get("EXP_NAME", "rspo_v2_selfheal")

    curr_path = pathlib.Path(__file__).resolve().parent.parent
    tune.run(
        "RSPOPPO",
        name="RSPO_V2_Tracking_SelfHeal_" + exp_name,
