import argparse
"""
QMIX (DQN) Test Script for Drone Swarm Coverage — Self-Healing
================================================================
Tests a trained QMIX Self-Healing checkpoint with visual rendering.

Usage:
    python test_qmix_cnn_cov.py
"""
...
from train_qmix_cnn_cov import QMIXCNNModel
...
def env_creator(render_mode="human"):
    print("-------------------------- QMIX SELF-HEALING ENV CREATOR (TEST) --------------------------")
    N_AGENTS = 4
    ...
    # SELF-HEALING: fault injection OFF for clean test, but compensation ON (matches training)
...
if __name__ == "__main__":
    ...
    print("Please enter the absolute path to the QMIX Self-Healing checkpoint.")
    ...
    print(f"  QMIX SELF-HEALING TEST — 4 Drones on 25x25 Grid")
    print(f"  Battery: 125 Max (Deplete:-1 | Charge:+15)")
    print(f"  Self-Healing: ON (Compensation active)")
    ...
    print(f"\n{Colors.CYAN}{Colors.BOLD}{'='*60}")
    print(f"  QMIX SELF-HEALING TEST FINISHED — {step_count} steps")
    print(f"{'='*60}{Colors.RESET}")
