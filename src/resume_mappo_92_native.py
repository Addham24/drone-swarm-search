...
class BatteryMetricsCallback(DefaultCallbacks):
    ...
    def on_episode_step(self, *, episode, env_runner=None, worker=None, base_env=None, **kwargs):
        # Collect per-step battery info from each agent
    ...
    def on_episode_end(self, *, episode, env_runner=None, worker=None, base_env=None, **kwargs):
        # Log aggregated metrics — these appear as custom_metrics/ in TensorBoard
        ...
        # Pull coverage rate from the last info of any agent
        for agent_id in episode.get_agents():
            info = episode.last_info_for(agent_id)
...
def env_creator(args):
    ...
    env.reward_scheme = {
        ...
        "search_cell": 5.0,
        "done": 500.0,
        "reward_poc": 0.0,  # Disable POC scaling — every new cell is worth the same
    }
    ...
    # All drones start in a 2x2 cluster at the top-left corner (the base station)
...
if __name__ == "__main__":
    ...
    print(f"Native restore from: {CHECKPOINT_PATH}")
    ...
    tune.run(
        ...
        checkpoint_freq=10,
        storage_path="/Users/AdhamMotawi/Desktop/dsse_run/ray_res/" + env_name,
