...
def run(_run, _config, _log):
    ...
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(
            dirname(dirname(abspath(__file__))), "results", "tb_logs_final"
        )
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        # Loop training updates by batch_size_run to keep the updates-per-step ratio identical
        # when running in parallel mode (e.g. 8 updates when batch_size_run=8).
