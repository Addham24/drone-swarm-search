"""
RSPO Vanilla Test & Simulator Script
======================================
Tests a trained RSPO Vanilla checkpoint on the DSSE Coverage simulator with visual rendering.

Environment: Vanilla (fault injection ON, compensation rewards OFF).
Architecture: Full RSPO (FHE + DOVD + RAC).

Usage:
    python test_rspo_vanilla.py
"""
...
from train_rspo_vanilla import RSPOModel
...
def env_creator(render_mode="human"):
    print("-------------------------- RSPO VANILLA TEST ENV CREATOR --------------------------")
    N_AGENTS = 4
    ...
    # VANILLA: Fault injection ON, but NO teammate compensation rewards
...
if __name__ == "__main__":
    ...
    register_env("DSSE_Coverage_RSPO_Vanilla", lambda config: ParallelPettingZooEnv(env_creator(render_mode=None)))

    curr_path = pathlib.Path().resolve()
    print("Please enter the absolute path to your trained RSPO Vanilla checkpoint.")
    ...
    print(f"  RSPO VANILLA SIMULATOR TEST")
    print(f"  Grid: 25x25 | Agents: 4 Drones | Battery: 125 Max")
    print(f"  Architecture: FHE + DOVD + RAC  |  Env: Vanilla (No Compensation)")
    ...
    print(f"\n{Colors.MAGENTA}{Colors.BOLD}{'='*65}")
    print(f"  RSPO VANILLA TEST FINISHED — {step_count} steps")
    print(f"{'='*65}{Colors.RESET}")
