"""
VDN-DQN Training Script for Drone Swarm Coverage — Self-Healing
=================================================================
DQN with VDN-style cooperative reward mixing and self-healing features:
- Global Reward Wrapper (VDN mixing: alpha=0.7)
- Random fault injection (fault_prob=0.0005)
- Teammate crash compensation rewards (BONUS=2.5, HORIZON=100)
- Tanh activations (matching MAPPO architecture for fair comparison)
- Dueling DQN + Double Q-Learning + Prioritized Experience Replay

Usage:
    python train_qmix_cnn_cov.py
"""
...
class QMIXCNNModel(TorchModelV2, nn.Module):
    """
    CNN-based Q-network for VDN-DQN training.
    Processes the coverage matrix through CNN and position/battery vector through MLP,
    then outputs Q-values for each discrete action.

    Architecture mirrors the MAPPO CNNModel (Tanh activations) for a fair
    apples-to-apples comparison between the two algorithms.
    """

    def __init__(self, obs_space, act_space, num_outputs, model_config, name, **kw):
        print("QMIX OBSSPACE: ", obs_space)
        ...
        print(f"QMIX Grid size: {grid_size}, CNN Dense layer input size: {flatten_size}")
        ...
        # Q-value head: outputs one Q-value per action
        self.q_head = nn.Linear(256, num_outputs)
        # Value head for dueling architecture (V(s) baseline)
...
def env_creator(args):
    print("-------------------------- VDN-DQN SELF-HEALING ENV CREATOR --------------------------")
    N_AGENTS = 4
    ...
    # SELF-HEALING: fault injection ON, teammate compensation ON (defaults)
...
if __name__ == "__main__":
    ...
    ModelCatalog.register_custom_model("QMIXCNNModel", QMIXCNNModel)
    ...
    config = (
        ...
        .training(
            ...
            # Dueling DQN: separates V(s) from A(s,a) for stable Q-values
            dueling=True,
            ...
            # Slow target updates for stability with 4 simultaneous learners
            target_network_update_freq=10000,
            # Gradient clipping to prevent CNN saturation
            grad_clip=10.0,
            model={
                "custom_model": "QMIXCNNModel",
    ...
    print(f"\n{'='*60}")
    print(f"  VDN-DQN Self-Healing Training: {exp_name}")
    print(f"  Reward Mixing: VDN (alpha=0.7)")
    print(f"  Fault Injection: ON (p=0.0005)")
    print(f"  Teammate Compensation: ON (bonus=2.5, horizon=100)")
    print(f"{'='*60}\n")
