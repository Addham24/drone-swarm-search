"""
RSPO: Resilience-Aware Swarm Policy Optimization (Training Script)
=====================================================================
Novel MARL Algorithm for Self-Healing Drone Swarm Coverage Tasks.

Features Implemented:
  1. Fleet Health Embedding (FHE): Dedicated MLP encoder conditioning Policy & Value heads.
  2. Dual-Objective Value Decomposition (DOVD): Separate V_search and V_takeover heads with learned alpha gating.
  3. Resilience-Adaptive Clipping (RAC): State-conditioned clip parameter scaling during attrition events.

Usage:
    python train_rspo_cnn_cov.py
"""
...
class RSPOModel(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        **kw,
        ...
        print(f"[RSPO] Grid size: {grid_size}, Flatten size: {flatten_size}, Positions Dim: {positions_dim}")
        ...
        # Processes full vector (positions + battery + crash coords) to produce h_t in R^32
        ...
        # ── 4. Policy Network (conditioned on CNN + Linear + FHE) ──
        ...
        # Computes state-dependent clip offset from FHE: eps_t = 0.1 + 0.2 * rac_gate(h_t)
...
if __name__ == "__main__":
    ...
    print(f"[RSPO] Configured with {num_gpus} GPUs and {num_runners} environment runners.")
    ...
    try:
        exp_name = input("Exp name for RSPO run: ")
    except EOFError:
        exp_name = "v1"
