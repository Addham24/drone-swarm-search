"""Test: build algo, set state, check if weights actually change"""
...
# Minimal model and env (same as main script)
...
# Get weights BEFORE restore
policy = algo.get_policy("default_policy")
w_before = {k: v.clone() for k, v in policy.model.named_parameters()}
print(f"BEFORE - cnn.0.weight mean: {w_before['cnn.0.weight'].mean():.6f}")
...
print(f"\nCheckpoint cnn.0.weight mean: {policy_state['weights']['cnn.0.weight'].mean():.6f}")

# Method 1: set_state
policy.set_state(policy_state)

w_after = {k: v.clone() for k, v in policy.model.named_parameters()}
print(f"AFTER set_state - cnn.0.weight mean: {w_after['cnn.0.weight'].mean():.6f}")

match = torch.allclose(w_after['cnn.0.weight'], torch.tensor(policy_state['weights']['cnn.0.weight']))
print(f"Weights match checkpoint: {match}")

# Method 2: Try direct set_weights
print("\n--- Trying set_weights method ---")
policy.set_weights(policy_state['weights'])
w_after2 = {k: v.clone() for k, v in policy.model.named_parameters()}
print(f"AFTER set_weights - cnn.0.weight mean: {w_after2['cnn.0.weight'].mean():.6f}")
match2 = torch.allclose(w_after2['cnn.0.weight'], torch.tensor(policy_state['weights']['cnn.0.weight']))
print(f"Weights match checkpoint: {match2}")

# Check sync
print("\n--- Checking remote worker weights ---")
try:
    remote_weights = algo.workers.foreach_worker(lambda w: w.get_policy("default_policy").get_weights()["cnn.0.weight"].mean())
    print(f"Remote worker weight means: {remote_weights}")
    
    algo.workers.sync_weights()
    remote_weights_after = algo.workers.foreach_worker(lambda w: w.get_policy("default_policy").get_weights()["cnn.0.weight"].mean())
    print(f"After sync_weights: {remote_weights_after}")
